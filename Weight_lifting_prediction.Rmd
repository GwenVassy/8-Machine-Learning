...
```{r remove2, echo = FALSE, include=FALSE }
# remove columns where the proportion of NAs is higher than 90%
testing <- testing[, proportionNA < 0.9]
```
...
dim(data)
```
```{r small2, echo=FALSE, include=FALSE}
# remove first 6 columns, and all columns that have near zero variance in the data set
training <- training[ , -c(1:6,nearZeroVar(data))]
```
...
Our Test predictions are:
```{r}
predict(mod.rf.simple, testing)
```
